...
export async function enqueueWebhook(agentId: string, type: string, payload: unknown) {
  ...
  const body = JSON.stringify({ id: eventId, type, createdAt: new Date().toISOString(), payload });
  const signature = createHmac("sha256", env().WEBHOOK_SIGNING_SECRET ?? env().API_KEY_PEPPER).update(body).digest("hex");
  const job = { eventId, agentId, callbackUrl: agent.callbackUrl, body, signature, status: "PENDING", attempts: 0, nextAttemptAt: new Date(), createdAt: new Date() };
