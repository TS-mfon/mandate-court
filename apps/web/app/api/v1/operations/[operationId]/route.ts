...
export async function GET(request: Request, context: { params: Promise<{ operationId: string }> }) {
  try {
    ...
    const operation = await db.collection("operations").findOne({ operationId, agentId: agent.agentId }, { projection: { _id: 0 } });
    if (!operation) throw new ApiError(404, "Operation not found");
    const jobs = await db.collection("relayJobs").find({ operationId }, { projection: { _id: 0, actorAuthorization: 0 } }).sort({ createdAt: 1 }).toArray();
